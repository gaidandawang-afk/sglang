...
class Qwen3MoeForCausalLM(nn.Module):
    ...
    def generate_weight_name_filter(cls, logical_experts_map: Dict[int, List[int]]):
        """Select missing routed-expert checkpoint tensors for EPLB recovery."""
        ...
        weight_name_filter._sglang_eplb_reload_stats = reload_stats
        return weight_name_filter

    def finalize_npu_eplb_expert_reload(self) -> None:
        """Reject a filtered reload that left a fused expert incomplete."""

        for layer in self.model.layers:
            mlp = getattr(layer, "mlp", None)
            experts = getattr(mlp, "experts", None)
            finalize = getattr(experts, "finalize_npu_formatted_expert_reload", None)
