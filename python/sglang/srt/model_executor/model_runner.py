...
from sglang.srt.fault_tolerance.state import TopologySnapshot
...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def fault_tolerance_capture_topology(self) -> TopologySnapshot:
        return TopologySnapshot(
            ...
            dist_init_method=self._fault_tolerance_dist_init_method(),
            nccl_port=self.dist_port,
        )
    ...
    def fault_tolerance_health_check(self) -> None:
        ...
        torch.distributed.all_reduce(tensor, group=get_world_group().device_group)
        if tensor.item() <= 0:
            raise RuntimeError("Fault tolerance health collective returned invalid data.")
