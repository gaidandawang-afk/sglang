...
def test_nz_p2p_requires_post2_but_accepts_post2_dev_build():
    ...
    require_support = load_utils_functions(torch)["require_torch_npu_nz_p2p_support"]

    with patch.dict(sys.modules, {"torch_npu": SimpleNamespace(__version__="2.10.0")}):
        try:
            require_support()
...
def test_nd_full_expert_is_converted_before_formatted_slot_write():
    ...
    copy_to_formatted = load_utils_functions(torch)["copy_to_npu_formatted_tensor_"]
    destination = FakeTensor([0, 0, 0, 0], storage_offset=8, data_ptr=108)
