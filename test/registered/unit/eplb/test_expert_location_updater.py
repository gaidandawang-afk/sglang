...
class TestExpertLocationUpdaterAfterScaleDown(CustomTestCase):
    def test_gpu_per_node_uses_original_ep_size_after_rank_failure(self):
        old_metadata = MagicMock()
        old_metadata.ep_size = 4
        old_metadata.num_local_physical_experts = 2
        old_metadata.physical_to_logical_map_cpu = torch.zeros(
            (1, 8), dtype=torch.int64
        )

        new_metadata = MagicMock()
        new_metadata.physical_to_logical_map_cpu = torch.zeros(
            (1, 8), dtype=torch.int64
        )
        ...
        self.assertEqual(update_single_layer.call_args.kwargs["world_size"], 3)
        self.assertEqual(update_single_layer.call_args.kwargs["num_gpu_per_node"], 1)

    def test_expert_copy_prefers_a_live_source_rank(self):
        ...
        self.assertTrue(any("chosen_src_rank=2" in line for line in logs))
        self.assertTrue(all(op.peer != 0 for op in fake_ops))

    def test_expert_p2p_reuses_original_device_group_and_rank_namespace(self):
        ...
        self.assertTrue(any("chosen_src_rank=2" in line for line in logs))
        self.assertTrue(fake_ops)
        self.assertTrue(all(op.peer == 2 for op in fake_ops))
        self.assertTrue(all(op.group is original_device_group for op in fake_ops))
