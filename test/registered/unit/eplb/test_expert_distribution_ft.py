...
def test_stat_dump_all_reduce_uses_cpu_survivor_group():
    ...
    method = load_method(
        ...
        {
            "_OutputMode": str,
            "_convert_global_physical_count_to_logical_count": lambda *args, **kwargs: (
                tensor
            ),
            "get_eplb_process_group_context": lambda: context,
            "torch": torch,
    ...
    assert tensor.cpu_called
    assert calls == [(tensor, {"op": "sum", "group": context.control_group})]
...
def test_average_utilization_broadcast_uses_compact_group_root_zero():
    ...
    assert calls == [(tensor, {"src": 0, "group": context.control_group})]
