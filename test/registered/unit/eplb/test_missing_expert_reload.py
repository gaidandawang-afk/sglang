...
def test_qwen_filter_selects_only_requested_routed_experts():
    ...
    assert weight_name_filter("model.layers.3.mlp.experts.7.gate_proj.weight")
    ...
    assert not weight_name_filter("model.layers.3.mlp.experts.8.gate_proj.weight")
    assert not weight_name_filter("model.layers.4.mlp.experts.7.gate_proj.weight")
    assert not weight_name_filter("model.layers.3.mlp.shared_experts.gate_proj.weight")
...
def test_qwen_mtp_inherits_the_model_owned_filter():
    ...
    cls = next(
        node
        for node in tree.body
        if isinstance(node, ast.ClassDef) and node.name == "Qwen3MoeForCausalLMMTP"
    )
    ...
    assert not any(
        isinstance(node, ast.FunctionDef) and node.name == "generate_weight_name_filter"
        for node in cls.body
    )
