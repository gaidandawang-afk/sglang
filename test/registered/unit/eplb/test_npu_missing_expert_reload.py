"""CPU tests for filtered Ascend EPLB missing-expert reloads."""
...
FUSED_MOE_PATH = REPO_ROOT / "python/sglang/srt/layers/moe/fused_moe_triton/layer.py"
...
class TestNpuMissingExpertReload(CustomTestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_filtered_reload_writes_in_place_and_finalizes(self):
        ...
        model = SimpleNamespace(
            load_weights=Mock(), finalize_npu_eplb_expert_reload=Mock()
        )
        weights = object()
        ...
        model.finalize_npu_eplb_expert_reload.assert_called_once_with()
    ...
    def test_qwen_filter_records_checkpoint_pair_coverage(self):
        ...
        self.assertTrue(weight_filter("model.layers.2.mlp.experts.64.gate_proj.weight"))
        self.assertTrue(weight_filter("model.layers.2.mlp.experts.65.down_proj.weight"))
        self.assertFalse(
            weight_filter("model.layers.3.mlp.experts.64.gate_proj.weight")
        )

        stats = weight_filter._sglang_eplb_reload_stats
    ...
    def test_reload_validation_rejects_unmatched_expert(self):
        ...
        with self.assertRaisesRegex(RuntimeError, r"unmatched_pairs=\[\(2, 65\)\]"):
            self.validate_reload(
    ...
    def test_formatted_w13_commits_only_after_both_halves(self):
        ...
        self.load_formatted_expert(owner, shard_id="w1", loaded_weight=gate, **kwargs)
        owner._commit_npu_formatted_expert_reload.assert_not_called()
        self.load_formatted_expert(owner, shard_id="w3", loaded_weight=up, **kwargs)
