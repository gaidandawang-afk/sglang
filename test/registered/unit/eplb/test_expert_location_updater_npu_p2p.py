...
class TestExpertLocationUpdaterNPUP2P(CustomTestCase):
    def test_non_npu_views_keep_existing_zero_copy_path(self):
        ...
        op = _FakeP2POp(torch.distributed.isend, tensor, peer=1)
    ...
    def test_staged_send_and_recv_have_zero_offset_and_copy_back(self):
        ...
        self.assertIs(staged_ops[0].group, survivor_group)
        self.assertIs(staged_ops[1].group, survivor_group)
        self.assertEqual([op.tag for op in staged_ops], [11, 12])
    ...
    def test_multicast_reuses_one_staged_send_tensor(self):
        send_tensor = torch.arange(12).reshape(3, 4)[1]
        ops = [
            _FakeP2POp(torch.distributed.isend, send_tensor, peer=1),
            _FakeP2POp(torch.distributed.isend, send_tensor, peer=2),
        ]
