...
class TestSchedulerFaultToleranceControl(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.run_dummy_dispatch = staticmethod(base_runner["run_dummy_via_model_runner"])
    ...
    def test_retry_does_not_recover_device(self):
        ...
        recover = (
            scheduler.tp_worker.model_runner.recover_npu_device_for_fault_tolerance_scale_down
        )
    ...
    def test_npu_scale_down_returns_failed_ack_when_dummy_batch_fails(self):
        scheduler = self.make_scheduler()
        scheduler.tp_worker.model_runner.run_npu_fault_tolerance_dummy_batch.side_effect = RuntimeError(
            "507015"
        )
        request = FaultToleranceCommandReqInput(
            request_id="s",
    ...
    def test_npu_scale_down_rebalances_before_updating_mc2_elastic_info(self):
        ...
        method_calls = [node for node in ast.walk(method) if isinstance(node, ast.Call)]
        rebalance = next(
            call
            for call in method_calls
            if isinstance(call.func, ast.Attribute) and call.func.attr == "rebalance"
        )
